...
async function updateClinic(req, res) {
  try {
    ...
    if (opdSeriesPrefix  !== undefined) {
      ...
      // Empty / cleared value is allowed - stored as null. The patient code
      // generator falls back to deriving a prefix from the clinic name in
      // that case (e.g. "Sharma Medical Clinic" -> SHA0001).
      const cleaned = String(opdSeriesPrefix).toUpperCase().replace(/[^A-Z0-9]/g, '').slice(0, 10);
      data.opdSeriesPrefix = cleaned || null;
    }
